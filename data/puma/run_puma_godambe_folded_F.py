...
if __name__ == '__main__':
    ...
    print("F_TX = {} ({}--{})".format(popt[4], np.exp(np.log(popt[4])-1.96*uncerts[4]), np.exp(np.log(popt[4])+1.96*uncerts[4])))
    print("F_FL = {} ({}--{})".format(popt[5], np.exp(np.log(popt[5])-1.96*uncerts[5]), np.exp(np.log(popt[5])+1.96*uncerts[5])))
